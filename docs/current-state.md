...
- A folder note may declare an ordered frontmatter `tabs:` list (ADR 0065; Atlas ADR-0096 H1) — each entry a mapping with a required stable lowercase slug `id`, a required `page` path to an authored composition note (resolved hub-folder-relative first, then vault-absolute), and an optional display `name` (fallback: page basename). A structurally valid declaration renders the space as a persistent top tab bar (`HubTabsView`) over the active tab's composition, mounted through `NoteView` full-page; the space header stays, the legacy note-body/frame-body/backlinks composition does not render. Only the active tab's composition is mounted at a time.
- Every declared tab is always visible in declaration order, wrapping on overflow — never a dropdown. The active tab persists as `SpaceDefinition.activeHubTab` in `.notidian/def.json`, never in frontmatter (Atlas ADR-0066 D4 config-vs-state split); a persisted id no longer declared falls back to the first tab.
- Classification is collision-safe: only a non-empty array containing mappings is a declaration attempt — scalars, string lists, and empty lists render the legacy page with no banner. An invalid attempt (missing/malformed/duplicate ids, missing pages, unknown keys) renders the legacy page plus a visible violation banner; it never partial-applies and never bricks the space. A valid tab whose page note is missing renders the embed error surface in the content area while the bar stays.
- Gated by the default-on `hubTabbedViews` kill-switch (Advanced settings); off renders every space as the legacy page regardless of declarations.
...
### Table Density, Column Descriptions, And Title-Cell Wrap

- A view can select a per-view row density (`predicate.rowDensity`, ADR-0096 H3 / Notidian-pb7p.3) from the view-options menu: `normal` (default) or `compact`, which tightens cell padding and the row-height floors so a dense hub tab fits more rows above the fold (measured live: median row height 51px to 37px on a 22-column database). The default is dropped on save, so an untouched view's stored predicate and rendered class list stay byte-identical.
- A column can carry an authored description, edited from the property menu and shown under the property name in the header hover tooltip — hubs carry elaboration in tooltips rather than body prose (Atlas ADR-0096 D1). The description is stored in the column's existing `attrs` JSON envelope (key `notidianDescription`), the same envelope text group order uses, because `m_fields` has no arbitrary extension columns and a new `SpaceProperty` field would be discarded on every database write.
- Setting a column's wrap mode to `wrap` now also wraps the File/page-title cell. That cell renders a path crumb whose own `nowrap`/ellipsis rules previously won the cascade, so the wrap toggle silently did nothing on the widest-text column; the override is scoped to wrap-mode cells only, and a CSS-source test guards both its presence and its scoping.
...
Read-only embeds are hard read-only for view configuration (Notidian-pb7p.2, Atlas ADR-0096 H2): the FilterBar hides every view-config write affordance (filter/sort/group-by chip rows, `+ Add Filter`, the inline view-settings buttons, and the layout/view-options menus — view search stays), and `savePredicate` applies read-mode changes to local render state only, never persisting them into the saved view schema. Row-edit policy is separately governed (ADR-0095 edit-as-capture) and is not restricted by this gate. A `bar: false` descriptor line additionally suppresses the entire view-config bar (title row, toolbar, and chips) for zero-chrome hub-tab compositions; absent or `bar: true` keeps the bar.

An embed block can add one or more `where:` lines (ADR-0066 Topic Hub v1 view mechanism; Notidian-ioxi) to overlay a conjunctive (AND) row filter onto the referenced view at render time — `=`/`!=`/`>`/`<`/`includes` plus relative-date `withinLast`/`olderThan`.

The target database's configured folder note can also own an ordered frontmatter `views:` list (ADR 0062). Each declaration has a stable lowercase `id`, an explicit native `base: { kind: table|view, id }`, and optional public-grammar `where` clauses. A `kind: view` embed resolves a matching declaration before native saved-view lookup; the declaration then resolves its explicit native base without recursively following other declarations. This works identically when the configured folder note lives inside the database folder or adjacent to it because resolution follows the indexed `space.notePath`, never a filename guess. A declaration id that collides with a native view id owns the embed resolution. A view embed with no matching declaration keeps the native path unchanged.

Native base filters, declaration clauses, and per-embed clauses compose in that order as logical AND. A declaration may also replace the native base's complete `sort`, `groupBy`, visible `columns`, positive `limit`, or display `kind` value for that render. Sort directions resolve through the live type-aware sort registry, and display kinds resolve through the live Notidian view registry rather than a copied allowlist. Every explicit field is checked against the resolved target schema; schema-hidden, system-only, unknown, duplicate, malformed, unsupported-type, and unsupported-layout requests render the escaped Notidian embed error surface and never mount a widened fallback.

All declaration and embed values are **read-path only**: `ContextEditorProvider` derives a projected predicate for rendering, while native predicate state and every `savePredicate`/`saveSchema` payload remain projection-free. Explicit rich values replace their corresponding whole native value; omitted values preserve the native value. The shared `renderPathViewOverlays` setting remains default **on** with an Advanced-settings kill switch; when off, filters and rich values are ignored and the native predicate passes through by identity.
...
| Database embed descriptor, legacy-ref parsing, `where:` grammar, and folder-note declared-view resolution | [notidianEmbed.ts](../src/core/utils/embeds/notidianEmbed.ts), [notidianEmbed.test.ts](../src/core/utils/embeds/notidianEmbed.test.ts), [notidianEmbed.where.test.ts](../src/core/utils/embeds/notidianEmbed.where.test.ts), [notidianDeclaredView.ts](../src/core/utils/embeds/notidianDeclaredView.ts), [notidianDeclaredView.test.ts](../src/core/utils/embeds/notidianDeclaredView.test.ts), [notidianDeclaredViewRuntime.ts](../src/core/utils/embeds/notidianDeclaredViewRuntime.ts), [notidianDeclaredViewRuntime.test.ts](../src/core/utils/embeds/notidianDeclaredViewRuntime.test.ts), [notidianCanvasEmbed.ts](../src/core/utils/embeds/notidianCanvasEmbed.ts), and [notidianCanvasEmbed.test.ts](../src/core/utils/embeds/notidianCanvasEmbed.test.ts) |
| Database embed renderer, read projection, save firewall, and Obsidian hosts | [NotidianEmbed.tsx](../src/core/react/components/NotidianEmbed/NotidianEmbed.tsx), [notidianMarkdownEmbed.tsx](../src/adapters/obsidian/utils/notidianMarkdownEmbed.tsx), [notidianEmbedCommands.tsx](../src/adapters/obsidian/utils/notidianEmbedCommands.tsx), [renderPathPredicateProjection.ts](../src/core/utils/contexts/predicate/renderPathPredicateProjection.ts), [renderPathPredicateProjection.test.ts](../src/core/utils/contexts/predicate/renderPathPredicateProjection.test.ts), [overlayFilters.ts](../src/core/utils/contexts/predicate/overlayFilters.ts), and [overlayFilters.test.ts](../src/core/utils/contexts/predicate/overlayFilters.test.ts) |
| Embed hard read-only: view-config write freeze, FilterBar affordance gating, and `bar: false` suppression | [ContextEditorContext.readModeFreeze.dom.test.tsx](../src/core/react/context/ContextEditorContext.readModeFreeze.dom.test.tsx), [FilterBar.readMode.dom.test.tsx](../src/core/react/components/SpaceView/Contexts/FilterBar/FilterBar.readMode.dom.test.tsx), and [NotidianEmbed.test.tsx](../src/core/react/components/NotidianEmbed/NotidianEmbed.test.tsx) |
| Table row density, column descriptions in header tooltips, and title-cell wrap | [tableRowDensity.ts](../src/core/utils/contexts/tableRowDensity.ts), [tableRowDensity.test.ts](../src/core/utils/contexts/tableRowDensity.test.ts), [propertyDescription.ts](../src/core/utils/contexts/propertyDescription.ts), [propertyDescription.test.ts](../src/core/utils/contexts/propertyDescription.test.ts), [ColumnHeader.tsx](../src/core/react/components/SpaceView/Contexts/TableView/ColumnHeader.tsx), and [TableView.wrap.css.test.ts](../src/core/react/components/SpaceView/Contexts/TableView/TableView.wrap.css.test.ts) |
| Tabbed hub views: `tabs:` declaration grammar, tab bar + composition mounting, and the SpaceInner seam | [hubTabs.ts](../src/core/utils/spaces/hubTabs.ts), [hubTabs.test.ts](../src/core/utils/spaces/hubTabs.test.ts), [HubTabsView.tsx](../src/core/react/components/SpaceView/HubTabsView.tsx), [HubTabsView.dom.test.tsx](../src/core/react/components/SpaceView/HubTabsView.dom.test.tsx), [SpaceInner.tsx](../src/core/react/components/SpaceView/SpaceInner.tsx), and [SpaceInner.dom.test.tsx](../src/core/react/components/SpaceView/SpaceInner.dom.test.tsx) |
