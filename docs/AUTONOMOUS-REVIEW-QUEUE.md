...
### Notidian-it0j — Frame root-prop / context-column TYPE-collision runtime divergence

> **Record:** bd `Notidian-it0j` (discovered-from `Notidian-9xbn`) — this doc entry
> is the renderer; the bead is the record and its close reason carries the evidence.
> This is the distinct RUNTIME layer under the sy30/9xbn FINGERPRINT fixes (closed
> by cb0c196), which stay untouched.

**2026-07-11 (`autolong/run-9`).** A stored, NON-EMPTY frame ROOT prop whose NAME
collides with a space context column typed `object-multi` (or `object`) inherited
that injected type ONLY on the EDITABLE topology (`linkProps` fed `[...tableData.cols,
...props.cols]` in `FrameEditorRootContext`) and not the READ topology (`frame.cols`
via `buildRootFromMDBFrame` / note embed). `generateCodeForProp` (executable.ts) then
compiled a MULTI-LINE array-literal value to the EXPRESSION form on the editable
surface (returns the array) but the STATEMENT form on the read/embed surface (returns
undefined) — the SAME stored prop rendering two different results by topology. Fixed
via the contained owner-triage option (b): execution IGNORES the injected
context-column type on such a non-column root prop so both topologies type it
identically and execute identically. Gated OFF because it is a core render-path change
that is NOT owner-requested and not fully offline-verifiable (committed ≠ deployed).

- **Setting:** `frameRootTypeConsistency` (default `false`) — `src/shared/types/settings.ts`,
  defaulted in `src/core/schemas/settings.ts`, OFF-default pinned in
  `src/core/schemas/settings.defaults.test.ts` (`DOCUMENTED_REVIEW_QUEUE_FLAGS`), with
  an `advanced`-category settings-UI toggle (`src/adapters/obsidian/settings.ts`,
  label in `src/shared/en.ts`).
- **What it does when ON:** `linkProps` (`src/core/utils/frames/ast.ts`) skips folding
  an injected context-column TYPE onto a root prop the node already owns with a
  NON-EMPTY authored value and no own type — so a name-colliding `object-multi`/`object`
  column no longer changes that authored prop's `generateCodeForProp` branch. A genuine
  column binding (empty root value `""`) and an author-typed root prop are untouched;
  stored values are never dropped.
- **What it does when OFF (default):** byte-for-byte legacy behavior — the injected type
  still lands on the editable path and the divergence is preserved (pinned by test).
- **How to toggle:** set `frameRootTypeConsistency: true` in the plugin's `data.json`
  (or flip the "Consistent Frame Root Prop Types" advanced toggle), then reload the
  plugin (`obsidian plugin:reload id=notidian`).
- **What to live-verify in the vault (the part gates can't cover):** because the vault
  ships only `$kit` frames, use a FIXTURE frame (the approach prior runs used) with a
  root prop whose value is a multi-line array literal (e.g. `[\n1,\n2\n]`) named to
  collide with an `object-multi` context column present in the editable table but not
  in the frame's own cols. With `hardenFrameExecution` ON, render that frame on the
  editable space-view surface AND on the read/embed surface (`![![Space#*frame]]`):
  - Flag ON → the prop renders IDENTICALLY on both surfaces.
  - Flag OFF → the divergence returns (array on editable, empty/undefined on embed),
    confirming the kill-switch restores legacy behavior.
- **Offline evidence in place:** `rootTypeConsistency.test.ts` — flag OFF preserves the
  legacy divergence (editable `[1,2]` vs read `undefined`); flag ON makes both topologies
  execute identically via a normalized type basis; the object-literal `{...}` case stays
  type-independent under both flag states; and the fix is surgical (genuine column
  bindings, author-typed props, and non-colliding columns keep their types).
  `settings.defaults.test.ts` pins the OFF default. Full suite (9937) + tsc + build green.
