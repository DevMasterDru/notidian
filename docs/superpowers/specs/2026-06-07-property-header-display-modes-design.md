...
The row-number gutter is not fixed-width CSS. It is computed from the largest visible row number: 24px for one digit, 30px for two digits, 36px for three digits, and so on. The row drag grip overlays the left side of the gutter so it does not force the row-number column wider while hidden.
