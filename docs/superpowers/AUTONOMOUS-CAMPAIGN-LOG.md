...
### e1u — Canonicalize wikilink→path in the shared relation resolver — DONE

- **The bug (Codex caught in the ahk review):** the rollup, sub-items, and back-relation runtime bridges each called the *pure* `resolvePath` (core/superstate/utils/path), which only handles `./ ../` relative links + alias stripping. A bare `[[Projects/Alpha]]` stayed `Projects/Alpha` (no `.md`, so it never matched the `Projects/Alpha.md` pathsIndex key) and a basename-only `[[Alpha]]` could not resolve at all — so all three relation features silently dropped real relations authored as plain `[[Note]]` frontmatter. The #1 live-verify caveat for the relations pillar.
- **Fix (once, in the shared layer):** new `makeRelationLinkResolver(superstate)` (relationResolver.ts) wraps `superstate.spaceManager.resolvePath`, which already tries the pure resolver first and then falls back to the primary adapter's link index (Obsidian's `getFirstLinkpathDest`) to canonicalize a linkpath+source into the actual file path. All three bridges (tableRollupRuntime, tableBackRelationRuntime, ContextEditorContext sub-items) now route through it; the duplicated `isSpace`/pure-`resolvePath` closures are gone.
- **Why this is safe:** `spaceManager.resolvePath` returns the original link unchanged when nothing resolves (never null), so a dangling `[[Missing]]` stays a stable non-matching key — rollup `count` still counts it (count is independent of resolution), value/numeric fns skip it. No behavior change for already-canonical full-path links.
- **Tests:** `tableRollupRuntime.test.ts` mock superstate gained a link-index-emulating `spaceManager.resolvePath` + a regression test proving `[[Tasks/A]]` (bare) and `[[B]]` (basename) resolve to `*.md` keys. New `tableBackRelationRuntime.test.ts` (5 tests) proves basename/bare back-links are found, incidental inlinks are excluded, and field aggregates work.
- **Gates:** 504/504 Jest (+6), tsc clean, build clean.
- **Live-verify on waking:** in a database with a relation column authored as plain `[[Note]]` (basename) frontmatter, the rollup/back-relation/sub-items columns should now populate (previously blank for basename/bare links).

### ucd — Ordered lifecycle select: state progression — DONE (Notidian side)

- **Scope split:** the bead has two halves. (1) *Ordered lifecycle states* — Notidian-owned. (2) *Status-toggle-from-review* (check a box on an Atlasidian ADR-206 review page → write back to the row) — that parsing/write-back lives in Atlasidian-69c, out of this repo. This commit delivers the Notidian half.
- **What already existed:** select option order is already meaningful end-to-end — `predicate/sort.ts` `optionSort`/`getOptionsOrder` sort by the options array order, the picker renders in that order, and `EditOptionsModal` lets you drag-reorder options. So the ordered-lifecycle *data model* was already complete; the gap was *progression* (advancing a row along the order).
- **New engine (pure, tested):** `optionLifecycle.ts` — `lifecycleValuesFromColumnValue(col.value)` parses the ordered option values; `stepLifecycleValue({values, current, direction, wrap})` steps one position. Empty/unknown current enters at the first (or last) state; the ends clamp by default (no silent retired→wishlist roll), `wrap:true` cycles. 14 unit tests.
- **Wiring (low-risk, additive):** TableView `onKeyDown` — with a single single-select option cell selected (not editing, no modifiers), `]` advances and `[` steps back. The computed value is written through the existing `pasteSelection` path, so it inherits the authority-aware transaction, the frontmatter conflict gate, and undo/redo for free. No-op for ranges, multi-select, source-backed, or non-option columns; `[`/`]` are not swallowed for other cell types (preventDefault only fires when a step actually applies).
- **Gates:** 518/518 Jest (+14), tsc clean, build clean. Doc: `docs/table-database-workflows.md` (shortcut table + lifecycle section).
- **Live-verify on waking:** open the Tools & Materials database (5-state status, e.g. trigger row [[Tribus Original Microbial Inoculant]]), select a status cell, press `]`/`[` → the status should step forward/back through the lifecycle order and persist to frontmatter; Cmd+Z undoes it.
- **Follow-ups (need live UI work / other repo):** a one-click chip-advance affordance and the Atlasidian toggle-from-review (69c) write-back are left for live sessions.
