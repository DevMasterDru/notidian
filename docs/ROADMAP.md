...
- **Retire the deprecated `autonomous-beads.js` fallback** — delete `.claude/workflows/autonomous-beads.js` after the global `long-autonomous-mode` engine is validated on Notidian once, and confirm the AGENTS.md "Long Autonomous Mode" section name is final (renamed twice this session). AGENTS.md rename + slim is already done; only owner-confirmation + post-validation deletion remain. — (`AGENTS.md` "Long Autonomous Mode" binding; bead `Notidian-9y6d`)
