...
- **Navigator text filter: optional content/full-text match (reads file bodies)** — pursue only if the owner asks after using the shipped name/path filter. — (`filterTreeByQuery`, `core/superstate/utils/spaces.ts`; ships from Notidian-nrjb; bead `Notidian-d6lk`)
- **Navigator text filter: debounce/optimize `filterTreeByQuery` for very large vaults** — pursue only if the owner reports typing lag. — (`filterTreeByQuery`, `core/superstate/utils/spaces.ts`; bead `Notidian-a9m7`)
