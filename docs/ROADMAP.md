...
- **Navigator text filter: optional content/full-text match (reads file bodies)** — pursue only after the base name/path filter (`Notidian-nrjb`) actually ships and the owner asks; nrjb's first attempt was reverted out of run-3 history (2026-07-05 drift audit: its stale `closed` status corrected back to open — `filterTreeByQuery` does not exist in the tree yet). — (planned home: `core/superstate/utils/spaces.ts`; base bead `Notidian-nrjb`; this bead `Notidian-d6lk`)
- **Navigator text filter: debounce/optimize `filterTreeByQuery` for very large vaults** — pursue only if the owner reports typing lag, after `Notidian-nrjb` ships the function (see previous line — not yet in tree). — (planned home: `core/superstate/utils/spaces.ts`; bead `Notidian-a9m7`)
