...
The left row gutter selects whole rows. Its width adapts to the largest visible row number, so one-digit views stay narrower than two- or three-digit views. Dragging from the row-number lane draws a marquee rectangle and selects every visible row the rectangle intersects. Dragging the overlay grip in the row gutter moves rows in the table order.
