...
Use the column menu's compact `Header Display` segmented control to decide how compact a header should be in the current view: `Adaptive`, `Icon + Text`, `Text Only`, or `Icon Only`. Icon-only headers can be resized down to the 18px sticker footprint; at that size, Notidian shows only the header sticker and keeps the full beautiful label available through the hover tooltip. Boolean/Yes-No columns use compact checkbox cell padding at that width, so the body cells do not force the column open again. Older 26px collapsed widths from previous builds are treated as 18px when loaded. The current header icon appears to the left of the header-name input at the top of the menu; click it to configure the icon, or use `Default` inside the icon picker to return to the field-type icon. These presentation settings are saved with the Notidian view and do not write frontmatter.
